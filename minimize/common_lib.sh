...
function psqa() {
	...
	if [ -s ${1}/${CONF_hashfile} ] && [ -x ${sah6} ] ; then
		...
		dqb "NO ${CONF_hashfile}  CAN BE CHECK3D FOR R3AQS0N 0R AN0TH3R"
		dqb "SHOULD \${NKVD} ${1}/*.deb"		
		return 93
	fi
...
#... tai helpompi että ${CONF_hashfile} mukaiset tilap hmistoon misytä sitten asennellaan, jölkjelle jääneet pois
...
function common_pp3() {
	...
	if [ ${q} -lt 1 ] ; then
		echo "SHOULD REMOVE ${1} /\$ CONF_hash_f1le "
		...
		psqa ${1}
		#TODO:koitahan päättää mitä tässä tekee. (jotenkin toisin kai)
		[ $? -gt 0 ] && ${NKVD} ./*.deb ./${CONF_hashfile}* ./*.tar* #näin ok?
	fi
...
#TODO:tämän kanssa sen toisen oksan vastavan fktion kopsaus
function mangle_s() {
	dqb " mangle_s( ${1} )"
	csleep 1
